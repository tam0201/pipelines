...
def create_pvc(wf_name, namespace):
    client = k8sutils.get_v1_client()
...
def is_pvc_exist(pvc_name, namespace):
    client = k8sutils.get_v1_client()
    try:
    # Try to get the PVC object
        pvc = client.read_namespaced_persistent_volume_claim(pvc_name, namespace)
    except kubernetes.client.rest.ApiException:
        # If the PVC doesn't exist, the API will raise an exception
        pvc_exists = False
    else:
        # If the PVC exists, the API will return the PVC object
        pvc_exists = True
    return pvc_exists

...
def rewrite_data_passing_to_use_volumes(
    ...
    * Passing constant arguments to artifact inputs is not supported for now. Only references can be passed.
    ...
    data_volume_name = volume.metadata.name
    ...
    for template in templates:
        ...
        if input_artifacts:
            input_parameters = template.get('inputs', {}).get('parameters', [])
        ...
        if output_artifacts:
            ...
            output_parameters = template.get('outputs', {}).get('parameters', [])
    ...
    for template in templates:
        ...
        for task in template.get('dag', {}).get('tasks', []) + [steps for group in template.get('steps', []) for steps in group]:
            ...
            if argument_artifacts:
                argument_parameters = task.get('arguments', {}).setdefault('parameters', [])
...
def transform_workflow(workflow: dict, volume:dict, path_prefix: str = 'artifact_data/') -> dict:
    ...
    print("Attaching PVC to Workflow")
